...
**Files**: `backend/tests/eval/golden_scenarios/`, `backend/tests/eval/eval_runner.py`, `backend/tests/eval/scoring.py`

**Tests**: Eval harness can load and replay all 15+ scenarios.
...
3. **Add lightweight LLM fallback** when rules + keywords are ambiguous (confidence < 0.7):
...
3. **Remove stage-aware sections from agent_prompts.yaml** that duplicate content already in per-stage YAMLs (e.g., "Requirements Checklist" duplicated between `agent_prompts.yaml` section 2.B and `clarification_planner.yaml`).
...
1. **Replace tool persistence model**:
   - AAA tools write directly to `pending_change_sets` + `artifact_drafts` tables
...
2. **Introduce canonical tool registry** (`backend/app/agents_system/tools/tool_registry.py`):
   - Central registry: maps stage → list of tools (with Pydantic input schemas)
   - Each AAA tool gets proper Pydantic `BaseModel` input schema
   - LangChain `bind_tools()` natively handles Pydantic schemas
   - All Lane B tools (B.1, B.4, B.5) register here
   - Delete `backend/app/agents_system/tools/tool_wrappers.py` once all tools are migrated

3. **Remove dead parsing code**:
   - Remove `AAA_STATE_UPDATE` regex extraction from `nodes/postprocess.py`
   - Remove `state_update_parser.py` heuristic merge
...
2. **Storage**: SQLite table `architect_profiles` (id, profile JSON, updated_at). One profile per installation (single-user tool — no `user_id` column).
   ...
   **Migration note**: New additive table via Alembic migration. No FK to `projects` — installation-level. No existing data affected. Strictly single-user in v1 — no multi-user schema hints.

3. **API endpoints**:
   - `GET /api/settings/architect-profile` — read current profile
   - `PUT /api/settings/architect-profile` — update profile

4. **Prompt injection**: `prompt_loader.py` injects a compact `ARCHITECT_PREFERENCES` block into the system prompt:
   ```
   ARCHITECT_PREFERENCES:
   - Primary region: eastus
   - IaC: bicep
   - Compliance: GDPR, SOC2
   - Cost ceiling: $5000/mo
   ```

5. **Frontend**: Settings page section for editing the architect profile (form with the fields above).

**New files**:
- `backend/app/features/settings/domain/architect_profile.py`
- `backend/app/features/settings/api/architect_profile_router.py`
- `frontend/src/features/settings/components/ArchitectProfileForm.tsx`

**Tests**:
- Profile CRUD (create, read, update)
- Prompt injection includes profile data
...
1. **New `project_notes` table**: id, project_id, category (decision | context | question | risk), content, source_message_id (optional — for "pin from chat" traceability), created_at, updated_at.
   ...
   **Migration note**: New additive table via Alembic migration. FK to `projects.id` with `ondelete='CASCADE'`. No existing data affected.
...
3. **Context injection**: Include recent notes as a compact context block in the system prompt (capped at 500 tokens).

4. **Frontend**: Notes tab in the left sidebar of the unified workspace. Quick-add from chat context menu ("Pin this as a note").

**New files**:
- `backend/app/features/projects/domain/project_notes.py`
...
- Context injection respects token cap
- Pin from chat creates note with source message reference
