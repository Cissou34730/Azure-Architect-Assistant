...
- Phase 12 cleanup removed the abandoned multi-agent specialist path and dead ReAct prompt shims; project chat now follows a single runtime path through stage classification, stage workers, the native tool-calling agent loop, and approval-time merge semantics.
- Stage routing now emits typed classification metadata (stage, confidence, source, rationale) into graph state, SSE stage events, and workflow_result.stageClassification; downstream consumers should use that contract instead of inferring intent from free-form text.
- Prompt assembly is split more cleanly: orchestrator routing text is only composed for the orchestrator path, while dedicated stage workers load prompt files directly and avoid `agent_prompts.yaml` fallback coupling.
...
- The unified workspace now also exposes a project-level quality gate tab backed by `backend/app/features/projects/application/quality_gate_service.py` and `GET /api/projects/{project_id}/quality-gate`; it reuses composed `ProjectState` data plus persisted `project_trace_events` to summarize WAF coverage, mindmap coverage, unresolved clarifications, missing delivery artifacts, and recent workflow activity without adding a new persistence table.
- Generic AAA tool calls now go through `backend/app/agents_system/tools/tool_registry.py`: the registry maps stage → allowed pending-change tools, `tool_wrappers.py` converts legacy `AAA_STATE_UPDATE` tool payloads into typed `pending_change_confirmation` results, and the wrapper records those change sets in the DB-backed pending-change tables before `postprocess.py` inspects tool observations. The runtime registry now only adds the standalone `azure_retail_prices` lookup path for pricing v1; the unified research facade stays internal to the research worker.
- Research/evidence handling is more deterministic now: `backend/app/agents_system/tools/research_tool.py` re-ranks project documents, KB sources, and Microsoft Learn results into a single evidence shape, and `nodes/research.py` passes those grounded excerpts/URLs into the architecture synthesizer handoff.
- Validation tool exposure is now explicit as well: `aaa_validate_mermaid_diagram` is only surfaced for general/candidate/validate turns, `aaa_validate_iac_bundle` is only surfaced for general/IaC turns, and both registrations live in the canonical runtime tool registry instead of ad hoc node wiring.
