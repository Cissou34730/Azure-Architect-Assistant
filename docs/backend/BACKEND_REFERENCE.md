...
- `nodes/agent_native.py` — Native LangGraph orchestrator node; builds system directives from the composed stage-aware prompt surface. When `artifact_edit_detected` is set in graph state, injects a mandatory artifact-persistence directive that maps each artifact type to its correct AAA tool, relaxes the MCP-first guardrail (persist first, research optional), adds document-search guidance, and enforces **output discipline** (never inline diagrams, requirements, questions, or architecture proposals as plain text — always persist via AAA tools, keep chat response to a concise summary). Agent iteration budget is now configurable via `chat_max_agent_iterations` in `LLMTuningSettingsMixin` (default 15, up from the prior hardcoded 10).
