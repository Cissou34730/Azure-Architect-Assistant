...
- `langgraph/graph_factory.py` — Project chat graph assembly; stage routing resolves before context summary/context-pack construction so stage-specific compaction sees the routed stage, `extract_requirements` has a dedicated runtime node, `propose_candidate` routes through a dedicated research-worker → architecture-planner slice, and `validate` now branches into a dedicated validate-stage worker before the generic agent path.
