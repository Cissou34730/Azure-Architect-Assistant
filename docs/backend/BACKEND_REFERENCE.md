...
- Clarification-answer turns now branch through `app.features.agent.application.clarification_resolution_worker.ClarificationResolutionWorker`, which packages resolved requirement updates, answered clarification-question statuses, and new assumptions into `_clarificationResolution` pending-change bundles; `app.features.projects.application.pending_changes_merge_service.PendingChangesMergeService` applies that command only during approval. **P8**: `clarify.py` adds `_user_wants_to_proceed()` to detect proceed-intent phrases ("proceed with defaults", "use defaults", "continue with assumptions") and routes them to `proceed_with_defaults()` rather than the LLM answer-extractor, persisting default assumptions as reviewable pending changes.
...
- **P4/P7 (candidate schema expansion)**: `AAAGenerateCandidateInput` and the `CandidateArchitecture` state model (both in `aaa_state_models.py`) now carry extended fields: `components`, `service_choices`, `alternatives_rejected`, `tradeoffs`, `risks`, `waf_mapping`, `nfr_mapping`, `cost_drivers`, `operational_considerations`, `security_controls`, `implementation_phases`, `adr_candidates`. All new fields are optional with empty defaults for backward compatibility. The `decision_evidence_map` field (P7) maps each service choice to the evidence packet IDs produced by the research node, enabling decision-to-evidence traceability. Old candidates without these fields still parse correctly (lax config, `extra="allow"`). The research node exposes `format_research_packet_ids_for_citation(packets)` to simplify referencing packet IDs in `decision_evidence_map`.
...
- `config/prompt_loader.py` — YAML prompt loader; loads `constitution.yaml` as the FIRST shared prompt file so global quality directives (thoroughness, completeness, persist-first, decisiveness, source grounding, no deflection, self-check) apply to ALL agent compositions. Supports both the legacy `agent_prompts.yaml` surface and modular prompt composition for stage-aware orchestrator prompts, exposes `load_prompt_file(...)` for file-only stage worker prompts, and truncates composed directives to the supplied context budget when one is provided.
...
- `memory/context_packs/stage_packers.py` — Stage-specific compaction builders; ADR packs read canonical `adrs`, validation packs summarize `wafChecklist.items[*].evaluations[*].status` from the current checklist payload, the context-pack runtime consumes `aaa_context_max_budget_tokens` (default 24K) as the pack assembly budget instead of reusing the compaction trigger threshold, Phase 11 turns `aaa_context_compaction_enabled` / `aaa_thread_memory_enabled` on by default, the `general` stage packer now includes requirements/assumptions/document summaries for artifact-edit turns, and document summaries are at priority 2 with 500-char truncation (no longer dropped first).
- `nodes/stage_routing.py` — Core stage enum, classification, retry logic. Now **async** to support LLM-based intent classification fallback. Classification uses a 3-layer defense for artifact intent detection: (1) keyword fast-path with modification verbs (`update`, `edit`, `modify`, `change`, `revise`, `refresh`) and creation verbs (`add`, `create`, `generate`, `state`, `suggest`, `propose`, `extract`, `come up with`), requiring both a matching verb AND an artifact target; (2) LLM-based intent classifier fallback when keywords miss (cheap call with system message, JSON mode, configurable 64 max_tokens / 5s timeout); (3) safe fallback to no-intent on any error. When artifact intent is detected, emits `artifact_edit_detected=True` as a graph state field. Rule guards route architecture-design to `propose_candidate`, review-style to `general`, IaC to dedicated stage, and fall back to state-gap routing.
- `nodes/intent_classifier.py` — LLM-based artifact intent classifier. Uses `AIService.chat()` with a system prompt, separate user message, and cheap token budget. Returns `ArtifactIntentResult(intent: bool, types: list[str])`. Configurable via `intent_classifier_max_tokens` (default 64) and `intent_classifier_timeout_seconds` (default 5.0) in `LLMTuningSettingsMixin`. Falls back to `ArtifactIntentResult(intent=False)` on LLM errors, timeouts, or malformed JSON.
...
- `nodes/agent_native.py` — Native LangGraph orchestrator node; builds system directives from the composed stage-aware prompt surface. When `artifact_edit_detected` is set in graph state, injects a mandatory artifact-persistence directive that maps each artifact type to its correct AAA tool, relaxes the MCP-first guardrail (persist first, research optional), and adds document-search guidance. An **always-on output discipline** directive (Layer 1) applies to ALL turns: forbids raw diagram/Mermaid markup, architecture dumps, and artifact lists inline while allowing summaries of existing artifacts. Agent iteration budget is now configurable via `chat_max_agent_iterations` in `LLMTuningSettingsMixin` (default 30). When `quality_retry_reason` is set in graph state (from the quality gate loop), the retry reason is injected into system directives so the agent can correct its output on the next pass.
...
- `features/agent/application/clarification_planner_worker.py` — Clarify-stage worker that turns canonical requirements, ambiguity markers, WAF gaps, mindmap gaps, and prior clarification history into grouped high-impact question sets without mutating canonical state. **P8**: each question now includes `affectedDecision`, `defaultAssumption`, and `riskIfWrong` fields in both the LLM schema and the serialized structured payload.
- `features/agent/application/clarification_resolution_worker.py` — Packages resolved requirement updates, answered clarification-question statuses, and new assumptions into `_clarificationResolution` pending-change bundles. **P8**: new `proceed_with_defaults()` method builds a change set directly from `defaultAssumption` fields on open questions without an LLM call, enabling the user to skip answering questions and accept defaults.
...
- `nodes/quality_gate.py` — Quality gate loop nodes (`completeness_check`, `build_quality_retry`). Checks whether the agent output is complete before persisting: if `artifact_edit_detected` but no AAA persistence tool was called, routes back to agent with retry reason. Error-prefixed or empty outputs also trigger retries. Max retries governed by `quality_retry_max` setting (default 2). Quality check sits BEFORE persist to prevent saving incomplete messages.
...
- `tools/tool_wrappers.py` — Single-input tool wrapper with structured error surfacing. Only retries on `TypeError` (signature-shape mismatch); all other exceptions return `"ERROR: tool {name} failed ({ExcType}): {message}"` to the agent so it can adapt. Prevents silent error swallowing and side-effect duplication.
