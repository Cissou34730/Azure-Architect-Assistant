...
- `langgraph/graph_factory.py` — Project chat graph assembly; stage routing resolves before context summary/context-pack construction so stage-specific compaction sees the routed stage, `extract_requirements` has a dedicated runtime node, `manage_adr` now branches into a dedicated ADR stage worker, `propose_candidate` routes through a dedicated research-worker → architecture-planner synthesizer slice, `validate` now branches into a dedicated validate-stage worker before the generic agent path, `pricing` now branches into a dedicated cost-stage worker that reuses the existing handoff + estimator nodes, and `export` now routes into a dedicated export-stage worker that reuses the AAA export tool instead of the generic agent loop.
