...
class MetricsCollector:
    ...
    def reset_histogram(self, name: str, labels: Optional[Dict[str, str]] = None) -> None:
...
def record_job_started(kb_id: str, job_id: str) -> None:
    ...
    collector.set_gauge('ingestion_job_active', 1.0, labels={'kb_id': kb_id, 'job_id': job_id})


def record_job_completed(kb_id: str, job_id: str, duration_seconds: float) -> None:
    """Record job completion."""
    collector = get_metrics_collector()
    collector.increment_counter('ingestion_jobs_completed_total', labels={'kb_id': kb_id})
    collector.set_gauge('ingestion_job_active', 0.0, labels={'kb_id': kb_id, 'job_id': job_id})
    collector.observe_histogram('ingestion_job_duration_seconds', duration_seconds, labels={'kb_id': kb_id})


def record_job_failed(kb_id: str, job_id: str, error_type: str) -> None:
    """Record job failure."""
    collector = get_metrics_collector()
    collector.increment_counter('ingestion_jobs_failed_total', labels={'kb_id': kb_id, 'error_type': error_type})
    collector.set_gauge('ingestion_job_active', 0.0, labels={'kb_id': kb_id, 'job_id': job_id})
...
def record_chunks_processed(kb_id: str, count: int) -> None:
    ...
    collector = get_metrics_collector()
    collector.increment_counter('ingestion_chunks_processed_total', count, labels={'kb_id': kb_id})


def record_queue_depth(kb_id: str, job_id: str, depth: int) -> None:
    """Record current queue depth."""
    collector = get_metrics_collector()
    collector.set_gauge('ingestion_queue_depth', float(depth), labels={'kb_id': kb_id, 'job_id': job_id})


def record_processing_latency(kb_id: str, operation: str, duration_seconds: float) -> None:
