...
class LinkExtractor:
    ...
    def extract_links(self, url: str, url_prefix: str) -> List[str]:
        ...
        for attempt in range(self.max_retries):
            try:
                logger.debug(
                    f"Extracting links from {url} (attempt {attempt + 1}/{self.max_retries})"
                )
                ...
                logger.debug(
                    f"Extracted {len(unique_links)} unique links from {url} (before dedup: {len(links)})"
                )
                ...
                if attempt < self.max_retries - 1:
                    logger.warning(
                        f"Failed to extract links: {e}, retrying in 2 seconds..."
                    )
                    time.sleep(2)
                else:
                    logger.error(
                        f"Failed to extract links after {self.max_retries} attempts: {e}"
                    )
                    return []
