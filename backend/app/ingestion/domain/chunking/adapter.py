...
def create_chunker_from_config(kb_config: Dict[str, Any]):
    ...
    logger.info(
        f"Creating chunker: strategy={strategy}, size={chunk_size}, overlap={chunk_overlap}"
    )
...
def chunk_documents_to_chunks(
    documents: List[Document], chunker, kb_id: str
) -> List[Chunk]:
    """
    ...
    for i, chunk_dict in enumerate(chunk_dicts):
        ...
        content_hash = compute_content_hash(
            text=content, kb_id=kb_id, source_id=source_id
        )
        ...
        chunks.append(Chunk(text=content, content_hash=content_hash, metadata=metadata))
