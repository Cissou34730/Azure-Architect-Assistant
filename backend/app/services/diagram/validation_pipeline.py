...
class ValidationPipeline:
    ...
    async def validate_diagram(
        ...
        if quality_report.warnings:
            logger.warning("Layer 3 (Quality) warnings: %s", "; ".join(quality_report.warnings))
    ...
    async def _check_visual_quality(
        ...
        if diagram_type != DiagramType.PLANTUML_AZURE:
            return await self.quality_checker.check_mermaid_visual_quality(diagram_source)
    ...
    def _build_semantic_retry_feedback(self, semantic_result: SemanticValidationResult) -> str:
