...
class DiagramGenerator:
    ...
    def __init__(self, llm_client: DiagramLLMClient) -> None:
        ...
        self.validation_pipeline = ValidationPipeline(
            llm_client=llm_client, plantuml_jar_path=None
        )
    ...
    async def generate_mermaid_functional(
        ...
        """
        logger.info(
            "Generating Mermaid functional diagram (description: %d chars)",
            len(description),
        )
    ...
    async def generate_c4_context(
        ...
        """
        logger.info(
            "Generating C4 Context diagram (description: %d chars)", len(description)
        )
    ...
    async def generate_c4_container(
        ...
        """
        logger.info(
            "Generating C4 Container diagram (description: %d chars)", len(description)
        )
    ...
    async def _validate_and_create_result(
        ...
        if validation_result.is_valid:
            logger.info("Diagram generated successfully on attempt %d", attempt)
            return (
                True,
                GenerationResult(
                    success=True,
        ...
        # Validation failed
        retry_feedback = (
            validation_result.retry_feedback or validation_result.error_message
        )
        logger.warning("Validation failed (attempt %d): %s", attempt, retry_feedback)
        return False, None, retry_feedback

    async def _generate_with_retry(
        self, description: str, diagram_type: DiagramType, max_retries: int
    ) -> GenerationResult:
