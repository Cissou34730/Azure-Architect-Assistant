...
from app.dependencies import get_ingestion_runtime_service_dependency

logger = logging.getLogger(__name__)


def _cancel_running_tasks(runtime_service: Any) -> None:
...
def _handle_ingestion_shutdown(
    ...
    logger.warning("Signal %s received - requesting ingestion shutdown", sig)
    runtime_service.shutdown_manager.request_shutdown()

    try:
        _cancel_running_tasks(runtime_service)
    except Exception as exc:  # noqa: BLE001
        logger.warning("Failed to cancel running ingestion tasks on signal: %s", exc)

    _chain_signal_handler(sig, frame, prev_handlers, _handle_ingestion_shutdown)


def install_ingestion_signal_handlers() -> None:
    """Ensure SIGINT/SIGTERM immediately request ingestion shutdown."""
    prev_handlers = {}
