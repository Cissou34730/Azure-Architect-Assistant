...
from app.features.diagrams.infrastructure.models import Diagram, DiagramSet, DiagramType
...
from .database import get_diagram_session
...
async def append_diagram_reference_to_project_state(
    ...
    if blob_state is None:
        await _project_state_store.ensure_record(project_id=project_id, db=db)
        blob_state = {}

    persisted = await compose_project_state(
