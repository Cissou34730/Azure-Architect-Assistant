...
class AISettingsMixin(BaseModel):
    ...
    ai_copilot_default_model: str = "gpt-5.2"
    ai_copilot_allowed_models: str = ""
    ai_copilot_request_timeout: float = Field(default=120.0)
    ai_copilot_startup_timeout: float = Field(default=30.0)
    ai_copilot_auth_poll_interval: float = Field(default=3.0)
    ai_copilot_auth_timeout: float = Field(default=300.0)
