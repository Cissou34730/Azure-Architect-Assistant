...
# Anchors - this file lives at backend/app/shared/config/settings/storage.py
...
_DERIVED_STORAGE_PATHS: Final[dict[str, str]] = {
    ...
    "models_cache_path": "models_cache.json",
    "runtime_ai_selection_path": "runtime_ai_selection.json",
...
def _default_data_root() -> Path:
    """Return the backend-local fallback data root.

    AppSettings is responsible for loading env-backed overrides from process
    env and .env. The mixin fallback stays structural so path resolution does
    not depend on direct env reads here.
    """
...
class StorageSettingsMixin(BaseModel):
    ...
    models_cache_path: Path | None = Field(
        default=None,
        description="Disk cache for provider model lists with 7-day TTL",
    )
    runtime_ai_selection_path: Path | None = Field(
        default=None,
        description="Persisted active LLM provider/model selection for restart-safe runtime overrides",
    )
