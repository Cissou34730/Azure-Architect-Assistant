...
class ResolveAmbiguityRequest(BaseModel):
    ...
    resolved: bool = Field(default=True, description="Mark as resolved (true) or unresolved (false)")
...
async def get_ambiguities(
    ...
    """
    logger.info("Fetching ambiguities for diagram_set_id=%s, resolved=%s", diagram_set_id, resolved)
    ...
    logger.info("Found %d ambiguities for diagram_set_id=%s", len(ambiguities), diagram_set_id)
...
async def resolve_ambiguity(
    ...
    logger.info(
        ...
        diagram_set_id, ambiguity_id, request.resolved
    )
    ...
    if not ambiguity:
        logger.warning("Ambiguity not found: %s (diagram_set_id=%s)", ambiguity_id, diagram_set_id)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
