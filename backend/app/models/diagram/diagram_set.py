...
class DiagramSet(Base):
    ...
    diagrams = relationship(
        "Diagram", back_populates="diagram_set", cascade="all, delete-orphan"
    )
    ambiguity_reports = relationship(
        "AmbiguityReport", back_populates="diagram_set", cascade="all, delete-orphan"
    )
    lock = relationship(
        "Lock",
        back_populates="diagram_set",
        uselist=False,
