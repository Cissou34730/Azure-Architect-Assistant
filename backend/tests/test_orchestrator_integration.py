...
class TestOrchestratorGates:
    ...
    async def test_pause_and_resume(self):
        ...
        mock_repo = MagicMock()
        mock_repo.get_job_status = MagicMock(side_effect=[
        ...
        mock_repo.get_job = MagicMock(return_value=JobView(
            id="job-1",
            kb_id="kb-1",
            status="running",
            checkpoint=None,
        ...
        mock_loader = AsyncMock(return_value=iter([[MagicMock(id="doc-1", text="content")]]))
        mock_chunker = MagicMock(return_value=[MagicMock(content_hash="hash1", text="chunk")])
...
class TestOrchestratorIdempotency:
    ...
    async def test_skip_existing_chunk(self):
        ...
        mock_repo.get_job_status = MagicMock(return_value="completed")  # Complete after one batch
        mock_repo.update_job = MagicMock()
        mock_repo.update_heartbeat = MagicMock()
