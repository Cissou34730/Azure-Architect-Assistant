...
All query logic (single KB and multi-KB orchestration) resides here. Index
loading/caching remains in `app.kb.service`.
"""
...
from app.kb.knowledge_base_manager import KBConfig, KBManager
...
class MultiSourceQueryService:
    ...
    def __init__(self, kb_manager: KBManager):
        ...
        self._kb_services: Dict[str, KnowledgeBaseQueryService] = {}
        logger.info("MultiSourceQueryService initialized")

    def _get_kb_service(self, kb_config: KBConfig) -> KnowledgeBaseQueryService:
        if kb_config.id not in self._kb_services:
            self._kb_services[kb_config.id] = KnowledgeBaseQueryService(kb_config)
